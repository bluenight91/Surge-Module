/* generated from lib/metrics.js + src/panel-main.js — do not edit by hand */
...
/**
 * Surge Panel summary for DNS-QUERY.
...
(function () {
  ...
  var stats = state.stats;
  var avg = Metrics.avgMs(stats);
  var content = [
    "查询 " + (stats.total || 0) + " · 成功 " + (stats.ok || 0) + " · 失败 " + (stats.fail || 0),
    "平均 " + avg + "ms · 最慢 " + (stats.maxMs || 0) + "ms",
    "EDNS " + (state.lastEdns || "-") + " · IP " + (state.publicIp || readStore(Metrics.PUBLIC_IP_KEY) || "-"),
    "面板 https://dns.query",
  ].join("\n");
